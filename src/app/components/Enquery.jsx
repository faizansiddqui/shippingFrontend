...
export default function Inquiry() {
    ...
    setFormData((prev) => {
      ...
      if (["length", "width", "height"].includes(name)) {
        const l =
          name === "length"
            ? parseFloat(value) || 0
            : parseFloat(newData.length) || 0;
        const w =
          name === "width"
            ? parseFloat(value) || 0
            : parseFloat(newData.width) || 0;
        const h =
          name === "height"
            ? parseFloat(value) || 0
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!rows) {
        const msg =
          (data && (data.message || data.error)) || "Unexpected order response";
        setErrorMsg(msg);
        setRates([]);
        return;
      }
      ...
      const mappedRates = rows.map((d) => {
        ...
        const payable =
          (Number.isNaN(backendTotal) ? baseFreight + gst : backendTotal) +
          surcharge;
        ...
        >
          ...
          <div className="space-y-3 mb-6">
            ...
            <div className="text-sm text-gray-600">
              {formData.parcelType.charAt(0).toUpperCase() +
                formData.parcelType.slice(1)}
            </div>
            <div className="text-sm text-gray-600">
              Weight: Volumetric {weightInfo.vol.toFixed(1)}kg | Actual{" "}
              {weightInfo.actual}kg
            </div>
          ...
          <div className="text-xs text-gray-500 mt-4">
            Estimated delivery days are based on average courier performance for
            the given pincodes.
          </div>
        </div>
      </div>
