  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        setMessage({
          type: "success",
          text: data.message || "Pickup address created successfully",
        });
        setFormData({
          address_name: "",
          contact_name: "",
      ...
      >
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-gray-500">
                Create a pickup location for pickup teams and RTOs.
              </p>
            </div>
          ...
          {formData.use_alt_rto_address && (
            <div className="bg-gray-50 rounded-xl p-4">
              ...
              <div className="grid sm:grid-cols-2 gap-4">
                ...
                />
                <input
                  name="create_rto_address.rto_gstin"
                  value={formData.create_rto_address.rto_gstin}
                  onChange={handleChange}
                  placeholder="GSTIN"
                  className="rounded-xl border p-3"
                />
        ...
        <aside className="bg-slate-50 p-4">
          ...
          <div className="space-y-3 text-sm">
            ...
            {formData.use_alt_rto_address && (
              <div className="mt-3 bg-gray-50 rounded p-3 text-xs">
                <div className="font-medium">
                  RTO — {formData.create_rto_address.rto_address_name || "—"}
                </div>
                <div>{formData.create_rto_address.rto_address_line || "—"}</div>
                <div>
                  {formData.create_rto_address.rto_city
                    ? `${formData.create_rto_address.rto_city} — ${formData.create_rto_address.rto_pincode || "—"}`
                    : "—"}
                </div>
                <div className="mt-2 text-gray-500">
                  RTO contact:{" "}
                  {formData.create_rto_address.rto_contact_number || "—"}
                </div>
              </div>
            )}
          </div>
          ...
          <p className="mt-3 text-xs text-gray-400">
            This preview doesn't affect saved data — it's just to help you
