...
export default function RechargeBalancePage() {
  ...
  async function fetchHistory() {
    try {
      ...
      const largePaiseCount = parsedTxs.reduce((c, t) => c + (Math.abs(t.amount) >= 10000 ? 1 : 0), 0);
      const assumePaise = parsedTxs.length > 0 && largePaiseCount > parsedTxs.length / 2;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-purple-50 via-white to-indigo-100 py-8">
      <div className="max-w-4xl mx-auto px-4">
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="text-right">
              ...
              <div className={`mt-1 px-3 py-1 rounded-full text-xs font-semibold ${
                walletBalance > 0 ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'
              }`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          <div className="bg-white rounded-xl shadow-md p-4 border border-green-100">
            <div className="flex items-center">
              <div className="p-2 bg-green-100 rounded-lg mr-3"><span className="text-green-600 text-lg">Up Arrow</span></div>
              <div>
          ...
          <div className="bg-white rounded-xl shadow-md p-4 border border-red-100">
            <div className="flex items-center">
              <div className="p-2 bg-red-100 rounded-lg mr-3"><span className="text-red-600 text-lg">Down Arrow</span></div>
              <div>
          ...
          <div className="bg-white rounded-xl shadow-md p-4 border border-blue-100">
            <div className="flex items-center">
              <div className="p-2 bg-blue-100 rounded-lg mr-3"><span className="text-blue-600 text-lg">Balance Scale</span></div>
              <div>
                <p className="text-sm font-medium text-gray-600">Net Balance</p>
                <p className={`text-xl font-bold ${netFromHistory >= 0 ? 'text-blue-600' : 'text-orange-600'}`}>
                  ₹{netFromHistory}
                </p>
              </div>
            </div>
        ...
        <p className="text-xs text-gray-500 text-center mb-6">
          Totals derived from transaction history. Server balance is authoritative.
        </p>

        {/* Recharge Form */}
        <div className="bg-white rounded-2xl shadow-xl p-6 mb-6">
          <h2 className="text-xl font-semibold text-gray-800 mb-4 text-center">Recharge Your Wallet</h2>
          <form onSubmit={handlePayment} className="space-y-4">
            ...
            <div className="flex justify-center">
              <button
                type="submit"
                disabled={submitting}
                className="bg-gradient-to-r from-purple-600 to-indigo-600 text-white px-8 py-3 rounded-xl font-semibold hover:from-purple-700 hover:to-indigo-700 disabled:opacity-50 transition-all shadow-md"
              >
                {submitting ? (
          ...
          {paginatedTransactions.length > 0 ? (
            <div className="space-y-3">
              {paginatedTransactions.map(tx => {
                ...
                const displayAmount = isDebit ? `-₹${Math.abs(amt).toFixed(2)}` : `+₹${amt.toFixed(2)}`;
                return (
                  <div key={tx.id} className="flex justify-between items-center p-4 bg-gray-50 rounded-xl border-l-4 border-l-purple-500 hover:bg-gray-100">
                    <div className="flex-1">
                      <div className="font-medium text-gray-800">{tx.description || "Wallet transaction"}</div>
                      <div className="text-xs text-gray-500">{new Date(tx.created_at).toLocaleString()}</div>
                      {tx.payment_id && (
                        <div className="text-xs text-purple-600 mt-1">ID: {tx.payment_id.slice(-8)}</div>
                      )}
                    </div>
